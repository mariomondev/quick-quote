  ...
  const generatePDF = () => {
    startTransition(async () => {
      try {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(...black);
        doc.text(formatShortDate(quote.created_at), detailValX, detailY, {
          align: "right",
        });
        ...
        doc.setFontSize(8.5);
        doc.setTextColor(...black);
        doc.text(formatCents(quote.total_cents), detailValX, detailY, {
          align: "right",
        });
        ...
        quote.line_items.forEach((item, index) => {
          ...
          doc.setTextColor(...darkGray);
          doc.text(item.quantity.toString(), col.qty, dataY, {
            align: "center",
          });
          doc.text(formatCents(item.unitPrice), col.price, dataY, {
            align: "right",
          });
          doc.setFont("helvetica", "bold");
          doc.setTextColor(...black);
          doc.text(formatCents(item.total), col.total, dataY, {
            align: "right",
          });
        ...
        doc.setTextColor(...black);
        doc.text(formatCents(quote.total_cents), totalsValueX, y, {
          align: "right",
        });
